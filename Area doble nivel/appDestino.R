...
#### DATOS ####
original <- read_excel("Destino export.xlsx")
...
servidor <- function(input, output) {
  ...
  datos_re <- reactive({
    if (botoncito()) {
      select(datos, Valor = Pje, Producto, Anio, texto, Total, Continente)
    } else {
      datos
    }
  })
  ...
  titY <- reactive({
    if (botoncito()) {
      "Porcentaje"
        ...
        "Miles de millones de dólares corrientes FOB"
      }
  })
  ...
  output$area <- renderPlotly({
    ...
    z <- ggplot(data = datos_re()) +
      aes(x = Anio, y = Valor, fill = Continente, text = texto, group = Producto) +
    ...
    salida_ply <- estetica(
      grafico = z, 
      tit = "Destino de Exportaciones", 
      subtit = "Miles de millones de dólares corrientes FOB",
      posx = 1998
    )
  ...
  output$id_descarga <- downloadHandler(
    filename = "Guate_en_Datos_Origen_Import.csv",
